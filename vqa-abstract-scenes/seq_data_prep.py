...
def get_questions(questions):
    ...
    for q in questions:
        ...
        for i in range(len(q)):
            token = tokens[q[i]]
            questions_bag[count][i] = token
        count += 1
    # print('questions bag ', questions_bag)
    return questions_bag, num_words
...
def set_data(check_top_ans, removeYesNo):
    ...
    # get questions bow
    # train
    train_questions_bag, num_words = get_questions(train_questions)
    ...
    y_test = y[ratio:]
    # split questions
    train_questions = train_questions_bag[:ratio]
    test_questions = train_questions_bag[ratio:]

    return x_train, x_test, y_train, y_test, train_questions, test_questions, all_ans, num_words

def seq_get_data(check_top_ans, removeYesNo):
